...
public class ShareService {
    ...
    @Transactional
    public ShareDtos.CreateShareResponse createShare(Users user, ShareDtos.CreateShareRequest req) {
        FileMetaData node = fileRepo.findOwnedActive(req.nodeId(), user)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "File not found"));

        if (node.isFolder()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Folders cannot be shared yet");
